...
public class JsonDatabaseManager {
    ...
    public void editLessonInCourse(String courseId, Lesson updatedLesson) {
        if (courseId == null || courseId.trim().isEmpty() || updatedLesson == null) {
            return;
        }
    ...
    public void deleteLessonFromCourse(String courseId, String lessonId) {
        if (courseId == null || courseId.trim().isEmpty() || lessonId == null || lessonId.trim().isEmpty()) {
            return;
        }
    ...
    public void markLessonCompletedInCourse(String courseId, String studentId, String lessonId) {
        ...
        // Check quiz completion only if a quiz exists (single-quiz model)
        if (!lesson.getQuiz().isEmpty()) {
            ...
            if (attempt == null || percentage < 50) {
                System.out.println("Cannot mark lesson completed: quiz not passed");
                return; // STOP here
            }
        }
    ...
    public boolean unenrollStudentFromCourse(String courseId, String studentId) {
        if (courseId == null || courseId.trim().isEmpty()
                || studentId == null || studentId.trim().isEmpty()) {
            return false;
        }
        ...
        // Remove lesson progress for the course
        if (course.getProgress().containsKey(studentId)) {
            course.getProgress().remove(studentId);
        }
        ...
        for (Lesson lesson : course.getLessons()) {
            if (!lesson.getQuiz().isEmpty()) {
                ...
                if (quiz != null && quiz.getQuizId() != null) {
                    student.getQuizAttempts().remove(quiz.getQuizId());
                }
            }
        }
        putUser(student);
    ...
    public void updateCourseAnalytics(String courseId, Map<String, Double> lessonCompletion, Map<String, Double> quizAverages) {
        ...
        course.setLessonCompletion(lessonCompletion);
        course.setQuizAverages(quizAverages);
