...
public class InstructorDashboardFrame extends JFrame {
    ...
    private JPanel createHeaderPanel() {
        ...
        analyticsBtn.setBackground(new Color(255, 165, 0));
        analyticsBtn.setForeground(Color.WHITE);
        analyticsBtn.setFocusPainted(false);
        analyticsBtn.setPreferredSize(new Dimension(120, 30));
        analyticsBtn.addActionListener(e -> openAnalyticsWindow());
    ...
    private JPanel createLessonsPanel() {
        ...
        addBtn.setBackground(new Color(70, 130, 180));
        editBtn.setBackground(new Color(60, 179, 113));
        editQuizBtn.setBackground(new Color(100, 149, 237));
        deleteBtn.setBackground(new Color(220, 80, 60));

        addBtn.setForeground(Color.WHITE);
        editBtn.setForeground(Color.WHITE);
        editQuizBtn.setForeground(Color.WHITE);
        deleteBtn.setForeground(Color.WHITE);
        ...
        editQuizBtn.addActionListener(e -> {
            Course selectedCourse = (Course) lessonCourseComboBox.getSelectedItem();
            Lesson selectedLesson = lessonList.getSelectedValue();
            if (selectedCourse == null || selectedLesson == null) {
                JOptionPane.showMessageDialog(this, "Please select a course and a lesson first.", "Selection Required", JOptionPane.WARNING_MESSAGE);
            ...
            openQuizEditor(selectedCourse, selectedLesson);
        });
    ...
    private void loadInstructorCourses() {
        ...
        for (String courseId : instructor.getCreatedCourses()) {
            ...
            if (c != null && c.isValidCourse()) {
                ...
                //add to combo boxes
                if (lessonCourseComboBox != null) {
                    lessonCourseComboBox.addItem(c);
                }
                if (studentCourseComboBox != null) {
                    studentCourseComboBox.addItem(c);
                }
    ...
    private void openQuizEditor(Course selectedCourse, Lesson selectedLesson) {
        QuizEditorFrame qf = new QuizEditorFrame(selectedLesson, selectedCourse, db);

        qf.addWindowListener(new WindowAdapter() {
            @Override
            public void windowClosed(WindowEvent e) {
                Course updated = db.getCourse(selectedCourse.getCourseId());
                if (updated != null) {
                    for (int i = 0; i < courseListModel.size(); i++) {
                        Course c = courseListModel.get(i);
                        if (c.getCourseId().equals(updated.getCourseId())) {
                            courseListModel.set(i, updated);
                            break;
                        }
                    }

                    if (lessonCourseComboBox != null) {
                        lessonCourseComboBox.removeAllItems();
                        for (int i = 0; i < courseListModel.size(); i++) {
                            lessonCourseComboBox.addItem(courseListModel.get(i));
                        }

                        lessonCourseComboBox.setSelectedItem(updated);
                    }
                    if (studentCourseComboBox != null) {
                        studentCourseComboBox.removeAllItems();
                        for (int i = 0; i < courseListModel.size(); i++) {
                            studentCourseComboBox.addItem(courseListModel.get(i));
                        }
                        studentCourseComboBox.setSelectedItem(updated);
                    }

                    // Refresh lesson list for the updated course
                    loadLessonsForCourse(updated);

                    if (selectedLesson != null) {
                        for (int i = 0; i < lessonListModel.size(); i++) {
                            Lesson ls = lessonListModel.get(i);
                            if (ls.getLessonId().equals(selectedLesson.getLessonId())) {
                                lessonList.setSelectedIndex(i);
                                break;
                            }
                        }
                    }
    ...
    private void deleteCourse() {
        ...
        if (confirm == JOptionPane.YES_OPTION) {
            ...
            if (lessonCourseComboBox != null) {
                lessonCourseComboBox.removeItem(c);
            }
            if (studentCourseComboBox != null) {
                studentCourseComboBox.removeItem(c);
            }
            lessonListModel.clear();
            studentListModel.clear();
            if (lessonContentArea != null) {
    ...
    private void openAnalyticsWindow() {
        ...
        if (selectedCourse == null) {
            JOptionPane.showMessageDialog(this, "Please select a course first to view analytics.",
                    "No Course Selected", JOptionPane.WARNING_MESSAGE);
            return;
        }
        Map<String, Double> lessonCompletion = new HashMap<>();
        ...
        for (Lesson lesson : selectedCourse.getLessons()) {
            ...
            for (String studentId : selectedCourse.getStudents()) {
                if (db.isLessonCompleted(selectedCourse.getCourseId(), studentId, lesson.getLessonId())) {
                    completedCount++;
                }
            }
            double completionPercent = selectedCourse.getStudents().isEmpty() ? 0
                    : (completedCount * 100.0 / selectedCourse.getStudents().size());
            lessonCompletion.put(lesson.getTitle(), completionPercent);

            if (lesson.getQuiz() != null && !lesson.getQuiz().isEmpty()) {
                Quiz quiz = lesson.getQuiz().get(0);
                double totalScore = 0;
                int attempts = 0;
                for (String studentId : selectedCourse.getStudents()) {
                    Student student = (Student) db.getUserById(studentId);
                    if (student != null) {
                        QuizAttempts qa = student.getLastQuizAttempt(quiz.getQuizId());
                        if (qa != null) {
                            totalScore += qa.getScore() * 100.0 / quiz.getQuestions().size();
                            attempts++;
                        }
                    }
                }
                double avgScore = attempts > 0 ? totalScore / attempts : 0;
        ...
        ChartFrame chartFrame = new ChartFrame("Analytics - " + selectedCourse.getTitle(),
                lessonCompletion, quizAverages);
