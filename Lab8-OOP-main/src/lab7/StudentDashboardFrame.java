...
public class StudentDashboardFrame extends JFrame {
    ...
    private JTabbedPane createMainPanel() {
        ...
        tabbedPane.addTab("Certificates", new StudentCertificatesPanel(student, db));
    ...
    private JPanel createEnrolledCoursesPanel() {
        ...
        unenrollBtn.setBackground(new Color(220, 80, 60));
        unenrollBtn.setForeground(Color.WHITE);
        unenrollBtn.addActionListener(e -> unenrollFromCourse());

        JButton startQuizBtn = new JButton("Start Quiz");
        startQuizBtn.setBackground(new Color(65, 105, 225)); // blue color
        startQuizBtn.setForeground(Color.WHITE);
        startQuizBtn.addActionListener(e -> startLessonQuiz());
    ...
    private void startLessonQuiz() {
        ...
        if (l.getQuiz() == null || l.getQuiz().isEmpty()) {
            JOptionPane.showMessageDialog(this,
        ...
        Quiz quiz = l.getQuiz().get(0);

        QuizFrame qf = new QuizFrame(quiz, student, new QuizServices(), l, c);
        qf.addWindowListener(new WindowAdapter() {
            @Override
            public void windowClosed(WindowEvent e) {
                
                User updated = db.getUserById(student.getUserId());
    ...
    private void loadEnrolledCourses() {
        ...
        for (String courseId : student.getEnrolledCourses()) {
            ...
            if (course != null && course.isValidCourse() && course.isApproved()) {
    ...
    private void loadLessons() {
        ...
        for (Lesson l : c.getLessons()) {
            ...
            boolean done = c.getProgress().get(student.getUserId()) != null
                    && c.getProgress().get(student.getUserId()).contains(l.getLessonId());

            Lesson displayCopy = new Lesson(
                    l.getLessonId(),
                    l.getTitle() + (done ? "  ✓ Completed" : ""),
                    l.getContent()
            );
            displayCopy.setQuiz(l.getQuiz());
            displayCopy.setResources(l.getResources());

            int idx = c.getLessons().indexOf(l);
            ...
            if (idx == 0) {
                ...
                Lesson prevLesson = c.getLessons().get(idx - 1);

                unlocked = c.getProgress().get(student.getUserId()) != null
                        && c.getProgress().get(student.getUserId()).contains(prevLesson.getLessonId());
            }

            if (!unlocked) {
                displayCopy.setTitle(displayCopy.getTitle() + "  (Locked)");
            }

            lessonListModel.addElement(displayCopy);
        }
    }
    ...
    private void unenrollFromCourse() {
        ...
        Course course = enrolledCoursesModel.getElementAt(selectedIndex);

        int confirm = JOptionPane.showConfirmDialog(
                this,
                "Are you sure you want to unenroll from \"" + course.getTitle() + "\"?\n"
                + "Your progress and quiz attempts for this course will be deleted.",
        ...
        boolean success = db.unenrollStudentFromCourse(course.getCourseId(), student.getUserId());

        if (!success) {
            JOptionPane.showMessageDialog(this,
                    "Failed to unenroll from \"" + course.getTitle() + "\".",
                    "Error",
        ...
        JOptionPane.showMessageDialog(this,
                "You have been unenrolled from \"" + course.getTitle() + "\".",
    ...
    private void markLessonCompleted() {
        ...
        if (!l.getQuiz().isEmpty()) {
            Quiz quiz = l.getQuiz().get(0);
            QuizAttempts attempt = student.getLastQuizAttempt(quiz.getQuizId());

            double percentage = (attempt != null)
                    ? (attempt.getScore() * 100.0 / quiz.getQuestions().size())
                    : 0;

            if (attempt == null || percentage < 50) {
                JOptionPane.showMessageDialog(this,
        ...
        student.markLessonCompleted(c.getCourseId(), l.getLessonId());
